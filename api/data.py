"""
Single source of truth for stats, reviews, and results shown on the site.

Edit the values below whenever you have real numbers/testimonials, then
commit + push -- Vercel will redeploy automatically. Nothing in this file
is ever sent to the browser directly; only the JSON built from it (via
index.py) is, so the raw data here stays out of page source / dev tools
Sources tab.
"""
